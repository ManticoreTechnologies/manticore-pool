...
var bignum = require('./bignumCompat.js');
...
var JobCounter = function () {
  ...
  counter_buf.writeBigUInt64BE(BigInt(counter), 24);
...
var JobManager = module.exports = function JobManager(options) {
  ...
  var maxShareValidationConcurrency = Math.max(parseInt(options.maxShareValidationConcurrency || 1, 10), 1);
  var maxShareValidationQueue = Math.max(parseInt(options.maxShareValidationQueue || 256, 10), maxShareValidationConcurrency);
  var activeShareValidations = 0;
  var shareValidationQueue = [];

  function runNextShareValidation() {
    while (activeShareValidations < maxShareValidationConcurrency && shareValidationQueue.length > 0) {
      activeShareValidations++;
      shareValidationQueue.shift()(function() {
        activeShareValidations--;
        runNextShareValidation();
      });
    }
  }

  function enqueueShareValidation(task) {
    if (shareValidationQueue.length >= maxShareValidationQueue) {
      return false;
    }

    shareValidationQueue.push(task);
    runNextShareValidation();
    return true;
  }
  ...
  this.processShare = function (miner_given_jobId, previousDifficulty, difficulty, miner_given_nonce, ipAddress, port, workerName, miner_given_header, miner_given_mixhash, extraNonce1, callback_parent) {
    ...
    var submitTimeMs = Date.now() || 0;
    //console.log('Submit Time: '+submitTimeMs);
    ...
    if (options.enforceNoncePrefix === true && miner_given_nonce.indexOf(extraNonce1.substring(0,4)) !== 0)
      return shareError([24, 'nonce out of worker range']);

    if (!job.registerSubmit(header_hash.toLowerCase(), miner_given_nonce.toLowerCase(), miner_given_mixhash.toLowerCase()))
    ...
    if (options.kawpow_validator == "kawpowd") {
      ...
      function(callback) {
          ...
          if (blockHash !== undefined) {
              ...
              const diff1_target = "00000000FFFF0000000000000000000000000000000000000000000000000000"
              const diff1_target_int = parseInt(diff1_target, 16);
              const blockHashHex = data.blockHash.toString('hex');
              const blockHashInt = parseInt(blockHashHex, 16);
              console.log('Block hash hex: '+blockHashHex);
              shareDifficulty = diff1_target_int / blockHashInt
              var shareDiff = shareDifficulty * shareMultiplier;
      ...
      var queued = enqueueShareValidation(function(validationDone) {
      _this.daemon.cmd('getevrprogpowhash', [ header_hash, miner_given_mixhash, miner_given_nonce, job.rpcData.height, job.target_hex ], function (results) {
        ...
        var rpcResult = results && results[0] ? results[0] : null;
        if (!rpcResult || rpcResult.error || !rpcResult.response) {
          var rpcError = rpcResult && rpcResult.error ? rpcResult.error : {message: 'missing RPC response'};
          emitWarningLog('Share validation RPC failed for ' + workerName + ': ' + JSON.stringify(rpcError));
          validationDone();
          return shareError([20, 'share validation rpc failed']);
        }

        var response = rpcResult.response;
        if (typeof response.digest === 'undefined' || typeof response.result === 'undefined') {
          emitWarningLog('Share validation RPC returned incomplete response for ' + workerName + ': ' + JSON.stringify(response));
          validationDone();
          return shareError([20, 'share validation rpc failed']);
        }
        ...
        var mix_hash = response.mix_hash || miner_given_mixhash;
        var meets_target = response.meets_target;
        ...
        if (isTrue(result)) {
          ...
          if (blockHash !== undefined) {
            ...
            //console.log("share diff: "+difficulty);
            shareDiffFixed = difficulty && difficulty.toFixed ? difficulty.toFixed(8) : difficulty;
            //console.log("SHARE DIFF: "+shareDiffFixed);
          }
          ...
          emitWarningLog(
            'Share validation mismatch for ' + workerName +
            ' height=' + job.rpcData.height +
            ' nonce=' + miner_given_nonce +
            ' submittedMix=' + miner_given_mixhash +
            ' daemonMix=' + response.mix_hash
          );
          validationDone();
      ...
      if (!queued) {
        emitWarningLog('Share validation queue full, rejecting share from ' + workerName);
        return shareError([21, 'share validation queue full']);
      }
    }
  }
};
