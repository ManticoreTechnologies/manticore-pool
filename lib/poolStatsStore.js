    ...
    tickHashWarsWorld(units, now) {
        ...
        world.territories.forEach(territory => {
            ...
            units.filter(unit => unit.territory === territory.name).forEach(unit => {
                ...
                const baseScore = unit[stance.weight] + (unit.energy * 0.2) + (unit.luck * 2);
            ...
            if (totalScore > 0) {
                HASH_WARS_FACTIONS.forEach(faction => {
                    ...
                    const momentumBonus = isOwner ? (1.0 + momentum * 0.5) : 1.0;
                    const gain = (scores[faction.id] / totalScore) * 18 * momentumBonus;
                    const baseDec = isOwner ? factionDef.decayRate : (4.0 * overstretch);
                    let decay = baseDec;
                ...
                HASH_WARS_FACTIONS.forEach(faction => {
                    ...
                    const decayAmount = hashrateDropped ? 3.0 : 1.5;
                    territory.control[faction.id] = clamp(current - decayAmount, 0, 100);
                });
            }
