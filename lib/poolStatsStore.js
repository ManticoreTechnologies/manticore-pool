...
const HASH_WARS_FACTIONS = [
    { id: 'forge', name: 'Forge', motto: 'Heat, pressure, output.', color: '#ff9d2e',
      trait: 'aggressive', spreadRate: 1.4, decayRate: 1.2, stability: 0.7 },
    { id: 'revenants', name: 'Revenants', motto: 'Back from stale work, hunting blocks.', color: '#b36bff',
      trait: 'predator', spreadRate: 1.1, decayRate: 0.8, stability: 0.9 },
    { id: 'iron-legion', name: 'Iron Legion', motto: 'Uptime is armor.', color: '#d7dde8',
      trait: 'fortified', spreadRate: 0.7, decayRate: 0.5, stability: 1.5 },
    { id: 'null-syndicate', name: 'Null Syndicate', motto: 'Silence, stealth, valid shares.', color: '#6cc7d8',
      trait: 'corrosive', spreadRate: 0.9, decayRate: 1.4, stability: 1.0 },
    { id: 'ghost-grid', name: 'Ghost Grid', motto: 'Distributed, persistent, unseen.', color: '#78f5a5',
      trait: 'distributed', spreadRate: 1.0, decayRate: 0.6, stability: 1.2 }
];

const HASH_WARS_TERRITORIES = [
    { name: 'Manticore Prime', buff: 'pressure_gen', buffValue: 1.12, buffLabel: '+12% pressure generation' },
    { name: 'Evrmore Gate', buff: 'block_amplify', buffValue: 1.5, buffLabel: 'Block shockwave x1.5' },
    { name: 'Sentinel Belt', buff: 'defense', buffValue: 1.2, buffLabel: '+20% defense' },
    { name: 'PeerWeave Drift', buff: 'spread', buffValue: 1.15, buffLabel: '+15% territory spread' },
    { name: 'Glyph Arena', buff: 'xp', buffValue: 1.25, buffLabel: '+25% XP gain' },
    { name: 'Vault Nebula', buff: 'decay_resist', buffValue: 0.6, buffLabel: '-40% decay rate' },
    { name: 'Null Expanse', buff: 'stealth', buffValue: 0.5, buffLabel: 'Stealth: -50% enemy detection' },
    { name: 'Forge Moon', buff: 'pressure_gen', buffValue: 1.18, buffLabel: '+18% pressure generation' },
    { name: 'Revenant Reach', buff: 'momentum', buffValue: 1.3, buffLabel: '+30% momentum bonus' }
];

const HASH_WARS_TERRITORY_NAMES = HASH_WARS_TERRITORIES.map(t => t.name);
...
const INVENTORY_ITEMS = [
    { id: 'entropy-shard', name: 'Entropy Shard', type: 'material', rarity: 'common', description: 'Raw entropy collected from valid shares. The basic currency of the war economy.' },
    { id: 'block-fragment', name: 'Block Fragment', type: 'material', rarity: 'epic', description: 'A shard of a found block. Extremely rare, proof of real network contribution.' },
    { id: 'shield-core', name: 'Shield Core', type: 'consumable', rarity: 'common', description: 'Grants +25% defense to your deployed sector for 1 tick.' },
    { id: 'pressure-bomb', name: 'Pressure Bomb', type: 'consumable', rarity: 'uncommon', description: 'Doubles your attack pressure in a sector for 1 tick.' },
    { id: 'warp-charge', name: 'Warp Charge', type: 'consumable', rarity: 'uncommon', description: 'Instantly redeploy a unit to any sector without cooldown.' },
    { id: 'overclock-cell', name: 'Overclock Cell', type: 'consumable', rarity: 'rare', description: 'Boosts hashrate contribution by 50% for 3 ticks.' },
    { id: 'territory-flag', name: 'Territory Flag', type: 'consumable', rarity: 'uncommon', description: 'Plants a flag granting +30% momentum bonus in a sector for 2 ticks.' },
    { id: 'decay-spike', name: 'Decay Spike', type: 'consumable', rarity: 'rare', description: 'Accelerates decay in an enemy-held sector for 2 ticks.' },
    { id: 'boss-trophy', name: 'Boss Trophy', type: 'trophy', rarity: 'rare', description: 'Awarded for contributing to a boss defeat.' },
    { id: 'territory-medal', name: 'Territory Medal', type: 'trophy', rarity: 'uncommon', description: 'Awarded when your faction seizes a new sector.' },
    { id: 'uptime-badge', name: 'Uptime Badge', type: 'trophy', rarity: 'uncommon', description: 'Awarded for 24 hours of continuous mining.' },
    { id: 'block-medallion', name: 'Block Medallion', type: 'trophy', rarity: 'epic', description: 'Awarded for personally finding a block.' },
    { id: 'titan-crest', name: 'Titan Crest', type: 'trophy', rarity: 'legendary', description: 'Awarded when a rig reaches Titan class.' }
];
...
class PoolStatsStore {
    ...
    load() {
        ...
        try {
            ...
            this.state.authTokens = parsed.authTokens || {};
    ...
    defaultWorker(workerName, password) {
        ...
        return {
            ...
            callsign,
            faction,
            territory: HASH_WARS_TERRITORY_NAMES[Math.abs(hashString(workerName)) % HASH_WARS_TERRITORY_NAMES.length],
            stance: 'assault',
            cosmetic: 'standard',
            password: password || '',
            valid: 0,
    ...
    setWorkerIdentity(workerName, identity) {
        ...
        if (Object.prototype.hasOwnProperty.call(identity, 'territory')) {
            const territory = HASH_WARS_TERRITORY_NAMES.find(item => item === identity.territory);
            if (territory) {
    ...
    authenticateAddress(address, password) {
        const normalized = String(address || '').trim();
        const pw = String(password || '');
        if (!normalized || !pw) return null;
        const workers = this.getWorkersByAddress(normalized);
        if (!workers.length) return null;
        const match = workers.find(w => w.password && w.password === pw);
    ...
    generateAuthToken(address) {
        ...
        if (!this.state.authTokens) this.state.authTokens = {};
        this.state.authTokens[token] = {
    ...
    validateAuthToken(token) {
        if (!this.state.authTokens || !token) return null;
        const entry = this.state.authTokens[String(token)];
        if (!entry) return null;
        if (Date.now() > entry.expiresAt) {
            delete this.state.authTokens[token];
            return null;
        }
    ...
    revokeAuthToken(token) {
        if (this.state.authTokens && token) {
            delete this.state.authTokens[String(token)];
            this.save();
        }
    }

    pruneAuthTokens() {
        if (!this.state.authTokens) return;
        const now = Date.now();
        Object.keys(this.state.authTokens).forEach(token => {
            if (now > this.state.authTokens[token].expiresAt) {
                delete this.state.authTokens[token];
            }
        });
    }

    grantItem(address, itemId, quantity, source) {
        const account = this.ensureAddress(address);
        if (!account.inventory) account.inventory = [];
        const itemDef = INVENTORY_ITEMS.find(i => i.id === itemId);
        if (!itemDef) return null;
        const qty = Math.max(1, Math.floor(safeNumber(quantity, 1)));
        const existing = account.inventory.find(i => i.id === itemId);
        if (existing) {
            existing.quantity += qty;
            existing.lastGrantedAt = Date.now();
        } else {
        ...
        if (!account.inventoryLog) account.inventoryLog = [];
        account.inventoryLog.unshift({
            itemId,
            quantity: qty,
            source: source || 'system',
            timestamp: Date.now()
        });
        account.inventoryLog = account.inventoryLog.slice(0, 50);
        this.save();
        return { itemId, name: itemDef.name, quantity: qty, rarity: itemDef.rarity };
    }

    useItem(address, itemId) {
        const account = this.ensureAddress(address);
        if (!account.inventory) return { error: 'No inventory' };
        const slot = account.inventory.find(i => i.id === itemId);
        if (!slot || slot.quantity <= 0) return { error: 'Item not found or depleted' };
        const itemDef = INVENTORY_ITEMS.find(i => i.id === itemId);
        if (!itemDef || itemDef.type === 'trophy' || itemDef.type === 'material') {
            return { error: 'This item cannot be consumed' };
        }
        slot.quantity -= 1;
        if (slot.quantity <= 0) {
            account.inventory = account.inventory.filter(i => i.id !== itemId);
        }
        this.save();
        return { used: true, itemId, name: itemDef.name, effect: itemDef.description };
    }

    getInventory(address) {
        const account = this.ensureAddress(address);
        return {
            items: (account.inventory || []).filter(i => i.quantity > 0),
            log: (account.inventoryLog || []).slice(0, 20)
        };
    }
    ...
    getAccountState(address) {
        ...
        workers.forEach(w => {
            const stats = this.getUnitStats(w, now);
            totalXp += stats.xp;
            totalHashrate += stats.hashrate;
            totalValid += stats.valid;
            totalBlocks += stats.blocks;
            if (stats.active) activeWorkers += 1;
        });
        return {
            address,
            xp: totalXp,
            level: Math.max(1, Math.floor(Math.log2(totalXp + 1))),
            hashrate: totalHashrate,
            validShares: totalValid,
            blocks: totalBlocks,
            workers: workers.length,
            activeWorkers,
            inventory: (account.inventory || []).filter(i => i.quantity > 0),
            inventoryLog: (account.inventoryLog || []).slice(0, 20)
        };
    }

    checkShareMilestones(address) {
        const account = this.ensureAddress(address);
        const workers = this.getWorkersByAddress(address);
        const totalValid = workers.reduce((sum, w) => sum + safeNumber(w.valid, 0), 0);
        const milestone = safeNumber(account.sharesMilestone, 0);
        const grants = [];
        const interval = 50;
        if (totalValid >= milestone + interval) {
            const earned = Math.floor((totalValid - milestone) / interval);
            account.sharesMilestone = milestone + earned * interval;
            grants.push(this.grantItem(address, 'entropy-shard', earned, 'share_milestone'));
            if (Math.random() < 0.12 * earned) {
                grants.push(this.grantItem(address, 'shield-core', 1, 'share_milestone'));
            }
            if (Math.random() < 0.06 * earned) {
                const rareItems = ['pressure-bomb', 'warp-charge', 'territory-flag'];
                const pick = rareItems[Math.floor(Math.random() * rareItems.length)];
                grants.push(this.grantItem(address, pick, 1, 'share_milestone'));
            }
        }
        return grants.filter(Boolean);
    }

    checkUptimeMilestones(address) {
        const account = this.ensureAddress(address);
        const workers = this.getWorkersByAddress(address);
        const now = Date.now();
        const grants = [];
        const activeCutoff = now - 5 * 60 * 1000;
        workers.forEach(w => {
            if (safeNumber(w.lastsharetime, 0) < activeCutoff) return;
            const uptimeMs = now - safeNumber(w.firstseen, now);
            const uptimeHours = uptimeMs / (60 * 60 * 1000);
            const lastMilestone = safeNumber(account.uptimeMilestone, 0);
            if (uptimeHours >= 24 && lastMilestone < 24) {
                account.uptimeMilestone = 24;
                grants.push(this.grantItem(address, 'uptime-badge', 1, 'uptime_24h'));
            }
            const hashrate = safeNumber(w.hashrate, 0);
            if (hashrate >= 1000000000 && !w._titanGranted) {
                w._titanGranted = true;
                grants.push(this.grantItem(address, 'titan-crest', 1, 'titan_class'));
            }
        });
        return grants.filter(Boolean);
    }
    ...
    recordShare(isValidShare, isValidBlock, shareData) {
        ...
        if (isValidShare) {
            this.checkShareMilestones(worker.address);
            this.checkUptimeMilestones(worker.address);
        }
    ...
    recordBlock(finder, shareData) {
        ...
        this.grantItem(finder.address, 'block-fragment', 1, 'block_found');
        this.grantItem(finder.address, 'block-medallion', 1, 'block_found');
        const bonusItems = ['overclock-cell', 'pressure-bomb', 'decay-spike', 'warp-charge'];
        this.grantItem(finder.address, bonusItems[Math.floor(Math.random() * bonusItems.length)], 1, 'block_reward');
    }
    ...
    getRigClass(hashrate) {
        ...
        if (rate >= 500000000) return 'Dreadnought';
        if (rate >= 200000000) return 'Siege Engine';
        if (rate >= 50000000) return 'Gladiator';
        if (rate >= 15000000) return 'Striker';
        if (rate >= 5000000) return 'Swarmer';
        return 'Scout';
    }

    getRigArchetype(worker, stats) {
        const variance = stats.invalid / Math.max(stats.valid + stats.invalid, 1);
        const burstiness = this.getShareBurstiness(worker);
        if (stats.blocks > 0 && stats.uptimeMinutes > 600) return 'Legendary';
        if (stats.hashrate > 100000000 && stats.efficiency < 0.92) return 'Berserker';
        if (stats.hashrate > 50000000 && burstiness > 0.7) return 'Phantom';
        if (stats.hashrate > 50000000 && stats.uptimeMinutes < 30) return 'Glass Cannon';
        if (stats.efficiency > 0.97 && stats.uptimeMinutes > 480) return 'Fortress';
        if (stats.efficiency > 0.97 && stats.uptimeMinutes > 240) return 'Tank';
        if (burstiness > 0.6) return 'Assault';
        if (burstiness < 0.25 && stats.valid > 20) return 'Pressure';
        if (stats.uptimeMinutes > 720 && stats.efficiency > 0.95) return 'Sentinel';
        if (stats.active && stats.valid < 5) return 'Ghost';
        return 'Line Unit';
    }

    getShareBurstiness(worker) {
        const times = (worker.historicalShareTimes || []).map(s => s.timestamp);
    ...
    ensureHashWarsWorld() {
        if (!this.state.hashWars) {
            ...
            this.state.hashWars = {
                ...
                territories: HASH_WARS_TERRITORIES.map((tDef, index) => {
                    ...
                    return {
                        id: tDef.name.toLowerCase().replace(/[^a-z0-9]+/g, '-'),
                        name: tDef.name,
                        owner: owner.id,
                        control,
                        buff: tDef.buff,
                        buffValue: tDef.buffValue,
                        buffLabel: tDef.buffLabel
                    };
                }),
                battleLog: []
            };
    ...
    generateHashWarsMap(seed) {
        ...
        const sectors = HASH_WARS_TERRITORY_NAMES.map((name, index) => {
            const ring = index % 3;
            const angle = ((Math.PI * 2) / HASH_WARS_TERRITORY_NAMES.length) * index + random() * 0.45;
    ...
    getAdjacentTerritories(territoryId) {
        ...
        const links = world.map.links || [];
        const adjacent = [];
        links.forEach(link => {
            if (link.from === territoryId) adjacent.push(link.to);
            else if (link.to === territoryId) adjacent.push(link.from);
        });
        return adjacent;
    }

    calculateMomentum(territory, world) {
        const adjacent = this.getAdjacentTerritories(territory.id);
        const adjacentOwned = adjacent.filter(adjId => {
            const adj = world.territories.find(t => t.id === adjId);
            return adj && adj.owner === territory.owner;
        }).length;
        const totalAdj = Math.max(adjacent.length, 1);
        const momentum = adjacentOwned / totalAdj;
        const overstretch = adjacentOwned === 0 ? 0.5 : 1.0;
        return { momentum, overstretch, adjacentOwned, totalAdj };
    }

    detectNetworkEvents(now) {
        const world = this.ensureHashWarsWorld();
        const events = [];
        ...
        if (history.length >= 2) {
            ...
            if (recent.difficulty && prev.difficulty) {
                const diffChange = (recent.difficulty - prev.difficulty) / Math.max(prev.difficulty, 1);
                if (diffChange > 0.05) {
                    events.push({ type: 'difficulty_spike', magnitude: diffChange, message: 'Network difficulty surging' });
                } else if (diffChange < -0.05) {
                    events.push({ type: 'difficulty_drop', magnitude: Math.abs(diffChange), message: 'Network difficulty collapsing' });
                }
            }
            if (recent.hashrate && prev.hashrate) {
                const hashChange = (recent.hashrate - prev.hashrate) / Math.max(prev.hashrate, 1);
                if (hashChange < -0.1) {
                    events.push({ type: 'hashrate_drop', magnitude: Math.abs(hashChange), message: 'Pool hashrate plummeting' });
                }
            }
        }
        const recentBlocks = (this.state.blocks || []).filter(b => now - safeNumber(b.submittedAt, 0) < 10 * 60 * 1000);
        if (recentBlocks.length > 0) {
            const finder = recentBlocks[recentBlocks.length - 1].finder;
            const worker = this.state.workers[finder];
            const faction = worker ? HASH_WARS_FACTIONS.find(f => f.id === worker.faction) : null;
            events.push({ type: 'block_found', faction: faction ? faction.id : null, factionName: faction ? faction.name : 'Unknown', message: `Block found by ${faction ? faction.name : 'unknown faction'}` });
        }
        return events;
    }
    ...
    tickHashWarsWorld(units, now) {
        const world = this.ensureHashWarsWorld();
        const tickInterval = 5 * 60 * 1000;
        if (world.lastTickAt && now - world.lastTickAt < tickInterval) {
            return world;
        }

        const networkEvents = this.detectNetworkEvents(now);
        const difficultySpike = networkEvents.find(e => e.type === 'difficulty_spike');
        const hashrateDropped = networkEvents.find(e => e.type === 'hashrate_drop');
        const blockFound = networkEvents.find(e => e.type === 'block_found');

        const previousOwners = {};
        world.territories.forEach(territory => {
            previousOwners[territory.id] = territory.owner;
            const { momentum, overstretch } = this.calculateMomentum(territory, world);

            const scores = {};
            const pressure = {};
            HASH_WARS_FACTIONS.forEach(faction => {
                scores[faction.id] = 0;
                pressure[faction.id] = 0;
            });

            const sectorBuff = territory.buff || '';
            const sectorBuffValue = safeNumber(territory.buffValue, 1.0);
            units.filter(unit => unit.territory === territory.name).forEach(unit => {
                const stance = HASH_WARS_STANCES.find(item => item.id === unit.stance) || HASH_WARS_STANCES[0];
                const factionDef = HASH_WARS_FACTIONS.find(f => f.id === unit.faction);
                const spreadRate = factionDef ? factionDef.spreadRate : 1.0;
                let baseScore = unit[stance.weight] + (unit.energy * 0.2) + (unit.luck * 2);
                if (sectorBuff === 'pressure_gen' && unit.faction === territory.owner) baseScore *= sectorBuffValue;
                if (sectorBuff === 'spread' && unit.faction === territory.owner) baseScore *= sectorBuffValue;
                if (sectorBuff === 'defense' && unit.faction === territory.owner && stance.id === 'defend') baseScore *= sectorBuffValue;
                scores[unit.faction] += baseScore * spreadRate;
                pressure[unit.faction] += unit.attack * spreadRate;
            });

            if (blockFound && blockFound.faction) {
                scores[blockFound.faction] = (scores[blockFound.faction] || 0) * 1.5;
                pressure[blockFound.faction] = (pressure[blockFound.faction] || 0) * 2.0;
            }

            const totalScore = Object.values(scores).reduce((sum, value) => sum + value, 0);
            if (totalScore > 0) {
                HASH_WARS_FACTIONS.forEach(faction => {
                    const factionDef = HASH_WARS_FACTIONS.find(f => f.id === faction.id);
                    const isOwner = faction.id === territory.owner;
                    let momentumMult = sectorBuff === 'momentum' && isOwner ? sectorBuffValue : 1.0;
                    const momentumBonus = isOwner ? (1.0 + momentum * 0.5) * momentumMult : 1.0;
                    const gain = (scores[faction.id] / totalScore) * 18 * momentumBonus;
                    let baseDec = isOwner ? factionDef.decayRate : (4.0 * overstretch);
                    if (sectorBuff === 'decay_resist' && isOwner) baseDec *= sectorBuffValue;
                    let decay = baseDec;

                    if (difficultySpike) decay *= 1.5;
                    if (hashrateDropped && !isOwner) decay *= 0.6;

                    territory.control[faction.id] = clamp(
                        safeNumber(territory.control[faction.id], 0) + gain - decay, 0, 100
                    );
                });
            } else {
                HASH_WARS_FACTIONS.forEach(faction => {
                    const current = safeNumber(territory.control[faction.id], 0);
                    const isOwner = faction.id === territory.owner;
                    const baseDecay = hashrateDropped ? 4.5 : 2.5;
                    const offlineDecay = isOwner ? baseDecay * 0.8 : baseDecay * 1.2;
                    territory.control[faction.id] = clamp(current - offlineDecay, 0, 100);
                });
                if (!territory._offlineDecayLogged && safeNumber(territory.control[territory.owner], 0) < 40) {
                    territory._offlineDecayLogged = now;
                    const ownerFaction = HASH_WARS_FACTIONS.find(f => f.id === territory.owner);
                    world.battleLog.unshift({
                        timestamp: now,
                        type: 'offline_decay',
                        message: `${territory.name} defenses collapsing — ${ownerFaction ? ownerFaction.name : 'owner'} miners offline`
                    });
                }
                if (territory._offlineDecayLogged && safeNumber(territory.control[territory.owner], 0) >= 50) {
                    delete territory._offlineDecayLogged;
                }
            }

            territory.pressure = pressure;

            const leader = HASH_WARS_FACTIONS
                .map(faction => ({ id: faction.id, value: safeNumber(territory.control[faction.id], 0) }))
                .sort((a, b) => b.value - a.value)[0];
            const runner = HASH_WARS_FACTIONS
                .map(faction => ({ id: faction.id, value: safeNumber(territory.control[faction.id], 0) }))
            ...
            const gap = leader && runner ? leader.value - runner.value : 100;
            territory.status = gap < 15 ? 'contested' : leader && leader.value < 45 ? 'unstable' : 'controlled';
            ...
            if (previousOwners[territory.id] !== territory.owner) {
                ...
                units.filter(u => u.faction === territory.owner && u.territory === territory.name).forEach(u => {
                    this.grantItem(u.address, 'territory-medal', 1, 'territory_flip');
                });
            }
        });

        networkEvents.forEach(evt => {
            if (evt.type !== 'block_found' || !world.battleLog.some(e => e.type === 'block_shockwave' && now - e.timestamp < 10 * 60 * 1000)) {
                world.battleLog.unshift({
                    timestamp: now,
                    type: evt.type === 'block_found' ? 'block_shockwave' : evt.type,
                    message: evt.message
                });
            }
        });
        ...
        if (validSinceStart >= world.eventTarget) {
            ...
            world.battleLog.unshift({
                timestamp: now,
                type: 'boss_defeated',
                message: `${faction ? faction.name : 'The pool'} breached the ${HASH_WARS_EVENTS[safeNumber(world.eventIndex, 0) % HASH_WARS_EVENTS.length].name}`
            });
            const topFactionId = topFaction && topFaction.faction;
            units.filter(u => u.faction === topFactionId).forEach(u => {
                this.grantItem(u.address, 'boss-trophy', 1, 'boss_defeated');
            });
    ...
    getHashWarsState() {
        ...
        const units = workers.map(worker => {
            ...
            const archetype = this.getRigArchetype(worker, stats);
            const unit = {
            ...
            faction.pressure += stats.attack + stats.energy * 0.3;
            faction.totalUnits += 1;
        ...
        const territories = world.territories.map(territory => {
            ...
            const { momentum, adjacentOwned, totalAdj } = this.calculateMomentum(territory, world);
            const controlMap = {};
            HASH_WARS_FACTIONS.forEach(f => {
                controlMap[f.id] = Math.round(safeNumber(territory.control[f.id], 0));
            });
            return {
                id: territory.id,
                name: territory.name,
                ...
                buff: territory.buffLabel || territory.buff,
                buffType: territory.buff,
                buffValue: territory.buffValue,
        ...
        factions.forEach(faction => {
            faction.territoriesOwned = territories.filter(t => t.owner === faction.id).length;
        });

        const frontlines = [];
        (world.map.links || []).forEach(link => {
            const fromTerritory = territories.find(t => t.id === link.from);
            const toTerritory = territories.find(t => t.id === link.to);
            if (fromTerritory && toTerritory && fromTerritory.owner !== toTerritory.owner) {
                frontlines.push({
                    from: link.from,
                    to: link.to,
                    x1: fromTerritory.x,
                    ...
                    x2: toTerritory.x,
                    y2: toTerritory.y,
                    factionA: fromTerritory.owner,
                    factionB: toTerritory.owner,
                    colorA: fromTerritory.ownerColor,
                    colorB: toTerritory.ownerColor,
                    intensity: Math.min(fromTerritory.control, toTerritory.control)
                });
            }
        });
        ...
        return {
            ...
            constants: {
                ...
                territories: HASH_WARS_TERRITORY_NAMES,
            ...
            totals: {
                ...
                pressure: units.reduce((sum, unit) => sum + unit.attack + unit.energy * 0.3, 0)
            }
        };
    }
...
module.exports.INVENTORY_ITEMS = INVENTORY_ITEMS;
