...
var StratumClient = function(options){
    ...
    function subscribeClient(message, callback){
        if (!_this.authorized) {
        ...
        _this.emit('subscription',
            ...
            function(error, extraNonce1, extraNonce2Size){
                ...
                callback(null, extraNonce1, extraNonce2Size);
            });
    }

    function handleSubscribe(message){
        subscribeClient(message, function(error, extraNonce1, extraNonce2Size){
            if (error){
                sendJson({
                    id: message.id,
                    result: null,
    ...
    function stripHexPrefix(value) {
        ...
        return value.indexOf('0x') === 0 || value.indexOf('0X') === 0 ? value.substr(2) : value;
    }

    function authorizeClient(id, username, password, callback){
        _this.workerName = getSafeWorkerString(username);
        _this.workerPass = password || 'x';
        ...
        options.authorizeFn(_this.remoteAddress, options.socket.localPort, _this.workerName, _this.workerPass, _this.extraNonce1, _this.version, function(result) {
            if (typeof result === 'boolean') {
                result = {
                    error: result ? null : [20, 'unauthorized worker', null],
                    authorized: result,
                    disconnect: false
                };
            }
    ...
    function handleAuthorize(message){
        ...
        authorizeClient(message.id, message.params[0], message.params[1], function(result) {
            sendJson({
                id     : message.id,
    ...
    function getLoginCredentials(params) {
        params = params || {};
        ...
        return {
            username: params.login || params.user || params.username || params.worker,
            password: params.pass || params.password || 'x'
        };
    }

    function handleLogin(message, ethStyle){
        var credentials = getLoginCredentials(message.params);
        if (!credentials.username) {
            sendJson({
                id: message.id,
                result: null,
                error: [20, 'missing login username', null]
            });
            return;
        }

        var finishAuthorization = function() {
            authorizeClient(message.id, credentials.username, credentials.password, function(result) {
                if (ethStyle) {
                    sendJson({
                        id: message.id,
                        result: _this.authorized,
        ...
        subscribeClient(message, function(error) {
            if (error) {
                sendJson({
                    id: message.id,
                    result: null,
    ...
    function handleConfigure(message) {
        var response = {};
        var requested = Array.isArray(message.params) ? message.params[0] : [];
        requested = Array.isArray(requested) ? requested : [];
        requested.forEach(function(extension) {
    ...
    function handleSuggestDifficulty(message) {
        var requestedDifficulty = Array.isArray(message.params) ? Number(message.params[0]) : NaN;
        if (isFinite(requestedDifficulty) && requestedDifficulty > 0) {
            _this.enqueueNextDifficulty(requestedDifficulty);
        }
        sendJson({
            id: message.id,
            result: true,
        ...
        _this.emit('submit',
            {
                ...
                nonce       : stripHexPrefix(message.params[2]),
                header      : stripHexPrefix(message.params[3]),
                mixhash     : stripHexPrefix(message.params[4])
            },
...
var StratumServer = exports.Server = function StratumServer(options, authorizeFn){
    ...
    (function init(){
        ...
        var bindAddress = options.bindAddress || options.stratumHost || options.remoteAddress || '0.0.0.0';
        var totalServers = Object.keys(options.ports).length;
        var handleServerStarted = function(port) {
            serversStarted++;
            _this.emit('log', 'special', 'Stratum listening on ' + bindAddress + ':' + port);
            if (serversStarted == totalServers)
                _this.emit('started');
        };
        var handleServerError = function(port, error) {
            _this.emit('log', 'error', 'Stratum failed to listen on ' + bindAddress + ':' + port + ' - ' + error.message);
        };
        Object.keys(options.ports).forEach(function(port){
            if (options.ports[port].tls) {
                ...
                tlsServer.on('error', function(error) {
                    handleServerError(port, error);
                });
                tlsServer.listen(parseInt(port), bindAddress, function() {
                    handleServerStarted(port);
                });
            } else {
              ...
              tcpServer.listen(parseInt(port), bindAddress, function() {
                  handleServerStarted(port);
              });
            }
        });
